...
/**
 Copied from https://raw.githubusercontent.com/diegonetto/generator-ionic/master/templates/hooks/after_prepare/update_platform_config.js
 */
        ...
        updateAndroidManifest: function (targetFile, configItems) {
            ...
            _.each(configItems, function (item) {
                ...
                if(item.type === 'preference') {
                    ...
                    // since there can be multiple uses-permission or uses-feature elements, we need to select them by unique name
                    if( 'uses-permission' === childSelector || 'uses-feature' === childSelector) {
                        childSelector += '[@android:name=\'' + data.attrib['android:name'] + '\']';
                    }

                    // find all elements and replace by the one from configItems
                    children = parentEl.findall(childSelector);

                    if(0 == children.length) {
                        // if no elements found, create new element
                        childEl = new et.Element(item.destination);
                        parentEl.append(childEl);
                    } else {
                        // else use first element (from configItems) and remove duplicate elements
                        childEl = children.shift();
                        _.each(children, function(el) { parentEl.remove(el); });
                    }
